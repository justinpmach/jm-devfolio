...
export default function ArtworkGrid() {
  ...
  return (
    <LayoutGroup>
      <ul className='min-h-screen w-full grid grid-cols-3 lg:grid-cols-4 gap-4 pt-20 md:p-8 lg:mt-36 xl:mt-40 justify-center items-center'>
        {artwork.map((item, idx) => (
          <motion.li
            className='relative bg-white p-[3rem] cursor-pointer h-[200px] lg:w-[200px] xl:w-[350px] rounded-md mx-auto'
            key={idx}
            layoutId={`${idx}`}
            onClick={() => {
              if (index === false) {
